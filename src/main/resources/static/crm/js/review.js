...
function renderReviewList(reviews) {
    const container = document.querySelector('.review-list-section');
    if (!container) return;
    
    const listContainer = container.querySelector('.review-list') || createReviewListContainer(container);
...
// 후기 목록 컨테이너 생성
function createReviewListContainer(container) {
...
// 별점 값 업데이트
function updateRatingValue(container, rating) {
...
// 별점 초기화
function resetStarRating(container) {
    const stars = container.querySelectorAll('.star-rating');
    stars.forEach(star => {
...
// Before 이미지 처리
function handleBeforeImage(input) {
